...
const now=new Date();const day=String(now.getDate()).padStart(2,'0');const weekday=now.toLocaleDateString('pl-PL',{weekday:'long'});const month=now.toLocaleDateString('pl-PL',{month:'long'});$('#todayBox').innerHTML=`<div class="date-daynum">${day}</div><div class="date-copy"><span class="date-weekday">${weekday}</span><strong>${month}</strong><span>${now.getFullYear()}</span></div>`;
